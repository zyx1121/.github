...
{ANSI Shadow: npx figlet-cli -f "ANSI Shadow" "{NAME}"
 ...
 block (terminal output / screenshot / GIF) instead; that's the stronger hook.}
```
...
CI validates every submission; once merged the site lists it and users install it with `/{plugin}:install <id>`. To contribute into the **official built-in set** instead, PR it directly under [`{plugin-dir}/components/<id>/`](…) here.
